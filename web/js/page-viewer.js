App.controller('viewer', (page, data) => {

  let $title = page.querySelector('.app-title'),
  ...
  let photoViewer = new PhotoViewer(page, urls, {
  ...
  page.addEventListener('appLayout', () => {
    let diff = (App.platform === 'android' ? 132 : 188);
  ...
  function setHeartIcon () {
    let image = data.images ? data.images[data.index] : data.image;
    if (Saved.has(image)) {
